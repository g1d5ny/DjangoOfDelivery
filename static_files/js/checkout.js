$(function () {
    ...
    $('.order-form').on('submit', function (e) {
        ...
        // 결제 정보 생성
        const merchant_id = AjaxStoreTransaction(e, order_id, amount, type);
...
function AjaxCreateOrder(e) {
    ...
    var request = $.ajax({
        ...
        data: $('.order-form').serialize() // serialize()를 통해 form의 객체들을 한번에 가능
    });
...
function ImpTransaction(e, order_id, merchant_id, imp_id, amount) {
    ...
    request.done(function (data) {
        if (data.works) {
            $(location).attr('href', location.origin + order_complte_url + '?order_id=' + order_id)
        }
    });
