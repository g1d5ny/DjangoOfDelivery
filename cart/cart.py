...
class Cart(object):  # 초기화, Django view에서 사용한 request로 그 안에 session 정보가 들어있음
    def __init__(self, request):
        ...
        self.cart = cart
        self.point_id = self.session.get('point_id')
    ...
    def __iter__(self):  # for 문 등 사용할 때 어떤 요소들을 건내줄 것인지 지정
        ...
        for item in self.cart.values():  # 제품 가격, Decimal : 숫자형으로 바꿔서 item에 넣어줌
            ...
            yield item  # Python generator 역할을 함, python 문법 (함수의 return과 유사하나 동작 방식이 조금 다름)
    ...
    def point(self):
        if self.point_id:
            return Point.objects.get(id=self.point_id)
        return None

    def get_discount_total(self):
        if self.point:
            if self.get_product_total() >= self.point.amount:
